(function(){
		...
		return new Promise(function(resolve,reject){
			...
			if(Object.keys(obj.data).length){
				...
				for(var i in data){
					...
					_this.appendTitle(data[i].EventTitle,i,data[i].EventDescription);

					if(data[i].Discussion){
						_this.appendClassDiscution([data[i].Discussion.Guid, data[i].Discussion.Title], i)
						_this.appendClassInGameActions([data[i].Discussion.Guid, data[i].Discussion.Title], i)
					}
	...
	Events.prototype.appendTitle = function(title, id, description){
		var html = '<div class="row">  <div class="col-md-12 event-wrapper-2">  <div class="class-discussion-wrapper"><h3 class="events-group-'+id+'">'+title+'</h3> <div class="class-options-wrapper"><div class="class-in-game-action-'+id+' class-in-game-action"></div><div class="class-discution-'+id+' class-discussion"></div></div></div> <div class="discussion-map-wrap map-'+id+'"></div> <div class="discussion-graph-wrap graph-'+id+'"></div><div class="discussion-chat-snippets-wrap chat-snippets-'+id+'"></div></div> </div> </div>';
		// var html = '<div class="row">  <div class="col-md-12 event-wrapper-2">  <div class="class-discussion-wrapper"><h3 class="events-group-'+id+'">'+title+'</h3><div class="class-discussion-description">'+description+'</div> <div class="class-options-wrapper"><div class="class-in-game-action-'+id+' class-in-game-action"></div><div class="class-discution-'+id+' class-discussion"></div></div></div> <div class="discussion-map-wrap map-'+id+'"></div> <div class="discussion-graph-wrap graph-'+id+'"></div><div class="discussion-chat-snippets-wrap chat-snippets-'+id+'"></div></div> </div> </div>';
	...
	Events.prototype.appendClassDiscution = function(className, id){
		...
		place.append('<a href="#" data-id="'+className[0]+'" class="eco-button submit-action discution_save" >Create Worksheet</a>');
	}

	Events.prototype.appendClassInGameActions = function(className, id){
		var place = $('.class-in-game-action-' + id);
		place.append('<a href="#" data-id="'+className[0]+'" class="eco-button submit-action in_game_action_save" >In-Game Actions</a>');
	}
	...
	Events.prototype.SaveDiscussionEvent = function(obj){
		return new Promise(function(resolve,reject){
			...
			$(document).on('click', '.in_game_action_save', function(){
				var Guid = $(this).attr('data-id'),
					data = obj.data,
					_this = obj.this;
				window.location.href = '/ingameaction.html?classId=' + Guid;
			});
		})
	}
