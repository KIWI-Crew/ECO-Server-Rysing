;(function(){
	...
	Discussion.prototype.appendTitle = function(title, description){
		var html = '<div class="section-wrapper"><div class="discussion-wrapper"><div class="global-title-small"><a href="#" class="edit" data-type="text" data-activities="false" data-name="title">'+title+' <i class="fa fa-pencil" style="display:none" aria-hidden="true"></i></a></div><p class="discussion-description-wrapper"><a href="#" class="edit" data-type="textarea" data-activities="false" data-name="description">'+description+' <i class="fa fa-pencil" style="display:none" aria-hidden="true"></i></a></p>';
				...
				if(_this.Discussion.Activities[i].Data.Maps.length){
					for(j in _this.Discussion.Activities[i].Data.Maps){
						...
						if(currentMaps){
							...
							data.Activities[i].Data.Maps[j].pause = currentMaps.pause;
	...
	Discussion.prototype.printDiscussionForm = function(){
		...
		iframe.onload = function() {
			var doc = iframe.contentDocument ? iframe.contentDocument : iframe.contentWindow.document;
				...
				var html = '<table style="width: 100%;font-family: Dosis;">'
					+'<tr style="display: block; margin:10px">'
						+'<td style=padding-left:10px;padding-right:10px;">Name: ______________________________________________</td>'
				...
				if(Object.keys(dataActivities[i]).length){
					if(dataActivities[i].Data.Graphs.length){
						for(j in dataActivities[i].Data.Graphs){
							...
							Graphs+='<td style="display: inline-block;padding:0 10px 20px;" colspan="3">'
					...
					if(dataActivities[i].Data.Maps.length){
						for(j in dataActivities[i].Data.Maps){
							...
							maps_count++;
							Maps+='<td style="display: inline-block;padding:0 10px 20px;" colspan="3">'
					...
					html+='<tr style="display: block; margin:80px 10px 0">'
							+'<td style="width: 98%;padding-left:10px;padding-right:10px;display: inherit;font-size:25px;font-weight:700;white-space: pre-wrap;" colspan="3"><span style="font-size:20px;">'+dataActivities[i].Format+':</span> '+dataActivities[i].Title.replace(/\n/g, "<br />")+'</td>'
						+'</tr>'
						+'<tr style="display: block; margin:10px">'
